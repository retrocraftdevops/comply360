...
func main() {
	...
	// Initialize service
	service, err := services.NewRegistrationService(repo, rabbitConn)
	if err != nil {
	...
	log.Printf("Registration Service starting on %s", addr)
	if err := r.Run(addr); err != nil {
		log.Fatalf("Failed to start server: %v", err)
	}
}

func setupRouter(db *sql.DB, handler *handlers.RegistrationHandler, jwtSecret string) *gin.Engine {
	// Set Gin mode
	...
	api := r.Group("/api/v1")
	{
		...
		{
			handler.SetupRoutes(registrations)
		}
	}
